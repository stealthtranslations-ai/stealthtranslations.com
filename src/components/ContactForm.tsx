...
export default function ContactForm() {
  ...
  useEffect(() => {
    ...
    if (position) {
      setTrackingInfo({ position, trackingId: tracking || '' });
      setFormData(prev => ({
        ...prev,
        service: 'Career Application',
        message: prev.message || `I am interested in the ${position} position.`
      }));
    }
  }, []);
  ...
  return (
    <div className="grid md:grid-cols-2 gap-8">
      ...
      >
        ...
        {trackingInfo.position && (
          <div className="bg-emerald-600/20 border border-emerald-500/30 rounded-lg p-4 mb-6">
            <h4 className="text-emerald-400 font-semibold mb-2">Career Application</h4>
            <p className="text-white mb-1">Position: <span className="font-medium">{trackingInfo.position}</span></p>
            <p className="text-gray-300 text-sm">Please complete the form below to apply for this position.</p>
            {trackingInfo.trackingId && (
              <p className="text-gray-400 text-xs mt-2">Tracking ID: {trackingInfo.trackingId}</p>
            )}
          </div>
        )}
